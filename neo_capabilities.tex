...
\begin{abstract}
Using the most recent prototypes, design, and as-built system information, we demonstrate and quantify the capabilities of the Large Synoptic Survey Telescope (LSST) to discover Near-Earth Objects (NEOs) and Potentially Hazardous Asteroids (PHAs), and show it may be possible
to improve the expected yields by modifying the baseline survey strategy.
...
an expected false positive detection rate using DECam data and prototype LSST software. We validate
that this expected rate is tractable with current prototype of the LSST Moving Object Processing System (MOPS). Using a high-fidelity 
simulated survey pointing history, we evaluate the performance of the LSST baseline survey strategy for 
PHAs and find an expected completeness of 68\% for PHAs with $H<22$ 
from LSST alone. We also generate variations on the baseline survey strategy to evaluate potential methods 
to increase the PHA completeness. We find that extending the LSST survey by two additional years and 
including previously known objects would increase the completeness for PHAs to 85\%. 
