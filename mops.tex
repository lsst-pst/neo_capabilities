...
by the \FindOrb code 
by Bill Gray\footnote{\FindOrb source code can be found at \url{https://github.com/Bill-Gray/find_orb/}}.
...
whose precision are representative of other analogous packages in the field (e.g. OpenOrb).
...
our 100ms computational budget. Running an afterburner using Herget's method
...
These findings are fully consistent with the results obtained here using the LSST implementation of MOPS. They confirm that the method linking method underlying both implementation is valid and robust. Furthermore, they empirically confirm a key assumption that we have made -- that the IOD is sufficient for filtering of admissible tracks.

Combined with our empirical measurement of the performance of LSST image differencing pipelines (in \S\ref{sec:imDiff}) and the LSST implementation of MOPS, the two studies provide strong evidence that LSST's approach to asteroid discovery is valid.