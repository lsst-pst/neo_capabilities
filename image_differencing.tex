...
\begin{figure}
  ...
  \caption{
  ``Stacked'' tracklets of three or more detections surrounding bright stars.
  $\Delta$ Dec and $\Delta$ RA are the tracklet coordinates relative to a nearby
  ...
  orientations. We do not see any such features, and the vast majority of
  tracklets are consistent with real moving objects. The ``hole'' in center of
  the plot comes from the rejection of \DIASources in regions with significant
  flux in the template image. 
  }
...
brighter than 14th magnitude using the UCAC4 catalog \citep{UCAC4}, and ``stacked'' the
