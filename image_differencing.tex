...
``science'' visits plus 5 template visits.

While this section presents statistics based on this subset of the Allen et al. data, we've subsequently verified the findings using a much larger superset of 540 visits from the same survey. This enlarged set was roughly bounded by $28^\circ < b < 58^\circ$ and $315^\circ < l < 350^\circ$ (Galactic coordinates), covering a range of object densities better representative of the LSST's "Wide-Fast-Deep" (WFD) survey (except for the depth, which we discuss in \S\ref{sec:imDiffScale}). Using this set, we find false detection rates broadly consistent with the data presented, with the additionally processed visits having fewer false detections by about 30\% (on average).
