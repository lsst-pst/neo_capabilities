...
\begin{figure}
  ...
  \caption{
  ``Stacked'' tracklets surrounding bright stars. $\Delta$ Dec and $\Delta$ RA
  are the tracklet coordinates relative to a nearby bright star, such that any
  correlated features due to (e.g.) bleed trails will show a grouping of lines
  with similar positions and orientations. We do not see any such features, and
  the vast majority of tracklets are consistent with real moving objects.
  }
...
For this test tracklets are required to have four detections (out of five visits
at each pointing) that align with velocities less than $2\deg$/day, and with
fixed spatial tolerances of $7\arcsec$ (XXX Clarify). We then identified star
brighter than 14th magnitude using the UCAC4 catalog, and ``stacked'' the
tracklets surrounding each bright star onto a single plot. This can be seen in
Figure~\ref{fig:stacked_tracklets}, where each black line corresponds to a
...
inspecting approximately 10,000 tracklets (the number in each plot is limited for
legibility).
